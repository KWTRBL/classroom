...
export default class BuildingData extends Component {
  ...
  componentWillMount() {
    axios.get('http://localhost:7777/teacherteach')
      .then(res => {
    ...
    axios.get('http://localhost:7777/teacherteachcurriculum')
      .then(res => {
        this.setState({
          curr2: res.data,
        })
  ...
  pageselectvalue(value) {
    ...
    this.setState({
      ...
      editlist: newId,
      tabledata: JSON.parse(this.state.olddata),
  ...
  async handleChange_editroom_status(index, event) {
    ...
    const newIds = this.state.tabledata //copy the array
    ...
    console.log(newIds)
    await this.setState({ tabledata: newIds }) //set the new state
  ...
  confirmedit = (index) => {
    ...
    axios
      .post("http://localhost:7777/teachdata/update", {
        year: this.state.tabledata[index].year,
        semester: this.state.tabledata[index].semester,
        curr: this.state.tabledata[index].curr2_id,
        dept: this.state.tabledata[index].dept_id,
        teachday: this.state.tabledata[index].teach_day,
        subject_id: this.state.tabledata[index].subject_id,
        teach_time: this.state.tabledata[index].teach_time,
        teach_time2: this.state.tabledata[index].teach_time2,
        teach_status: this.state.tabledata[index].teach_status,
      })
      .then(response => {
  ...
  render() {
    var editjson = []
    const item = this.state.tabledata.filter((member, index) => {
      if (this.state.yearsearch == null) {
        this.setState({ curr2search: "01", yearsearch: 2563, semestersearch: 1})
        if (member.dept_id == "01" && member.year == 2563 && member.semester == 1 ) {
          editjson.push(index)
          //console.log(editjson)
        }
        return member.dept_id == "01" && member.year == 2563 && member.semester == 1
      }
      else if ((member.dept_id == this.state.curr2search) && (member.year == this.state.yearsearch) && (member.semester == this.state.semestersearch) ) {
        editjson.push(index)
        //console.log(editjson)
        return member
      }
    ...
    let data_num = this.state.tabledata.filter((member) => {
      if (this.state.yearsearch == null) {
        this.setState({ curr2search: "01", yearsearch: 2563, semestersearch: 1})
        return member.dept_id == "01" && member.year == 2563 && member.semester == 1
      }
        ...
        <div className="content-wrap">
          ...
          <div id="detail">
            ...
            <Table striped responsive className="Crtable">
              <thead>
                <tr className="Teacherteachtable">
                  <th>รหัสวิชา</th>
                  <th>ชื่อวิชา</th>
                  <th>กลุ่ม</th>
                  <th>อาจารย์ผู้สอน</th>
