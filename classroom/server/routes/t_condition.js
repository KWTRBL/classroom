...
module.exports.updatecondition = function (req,callback){
    ...
    let sql = "UPDATE t_condition SET condition_status = ? ,building_no = ? ,own_subject = ?,condition_week = ?,condition_time = ?,condition_weekend = ? WHERE person_id = ?"  // คำสั่ง sql
    pool.getConnection((err, connection) => {
        if(err) throw err;
        pool.query(sql,[condition_status,building_no,own_subject,condition_week,condition_time,condition_weekend,person_id],(err, rows) => {
            if(err) throw err;
            console.log('The data from users table are: \n', rows);
            callback(rows)
            connection.release(); // return the connection to pool
