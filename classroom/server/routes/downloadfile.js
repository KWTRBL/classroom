...
module.exports.read = async function (req, callback) {
    ...
    teach_table.studentnum as จำนวนนศ,teach_table.room_no as ห้องเรียน,teach_table.building_no as อาคารเรียน,teach_table.teach_day as วันที่เรียน,teach_table.teach_time as เวลาเริ่มเรียน,teach_table.teach_time2 as เวลาเลิกเรียน,teach_table.lect_or_prac as ประเภท,teach_status as การจัดห้อง
    FROM teach_table ,curriculum2,subject,teacher,teacher_teach
    WHERE teach_table.semester=? AND teach_table.year=? AND teach_table.subject_id=subject.subject_id AND teach_table.curr2_id=curriculum2.curr2_id AND teach_table.subject_id=teacher_teach.subject_id AND teach_table.section=teacher_teach.section AND teacher_teach.teacher_id=teacher.teacher_id 
