...
module.exports.othersubject = async function (callback) {
  ...
  for (let faculty_index = 0; faculty_index < facultylist.length; faculty_index++) {
    ...
    for (
      ...
      // var subjectList = await getdataFromSql(`
      // SELECT t_exam_room.* FROM t_exam_room left JOIN t_exam_committee on t_exam_room.year = t_exam_committee.year and t_exam_room.semester = t_exam_committee.semester and t_exam_room.mid_or_final = t_exam_committee.mid_or_final and t_exam_room.exam_date = t_exam_committee.exam_date and t_exam_room.exam_time = t_exam_committee.exam_time and t_exam_room.subject_id = t_exam_committee.subject_id and t_exam_room.building_no = t_exam_committee.building_no AND t_exam_room.room_no = t_exam_committee.room_no WHERE t_exam_committee.year is null and t_exam_committee.semester is null and t_exam_committee.mid_or_final is null and t_exam_committee.exam_date is null and t_exam_committee.exam_time is null and t_exam_committee.subject_id is null and t_exam_committee.building_no is null AND t_exam_committee.room_no is null and t_exam_room.room_no not in(select DISTINCT room_no FROM t_exam_committee)
      // `)
      var subjectList = await getdataFromSql(
